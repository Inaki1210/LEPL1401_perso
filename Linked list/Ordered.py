...
class OrderedLinkedList(LinkedList):
    ...
    def insert(self, index, cargo):
        if index < 0:
            return "Index inférieur à zéro"
        
        elif index > self.size():
            return "Index plus grand que la liste"
            ...
            node_cargo = self.Node(cargo)
            node_cargo.next = node.next()
            node.next = node_cargo
    
    def add(self, pos, cargo):
        if pos == 1:
            self.add_first(cargo)
        elif pos == self.size():
            self.add_to_end(cargo)
        else:
            self.insert(pos,cargo)
    ...
    def remove(self, index):
        ...
        else: # retirer élément random
            pass
    

#test gpt
oll = OrderedLinkedList([3, 1, 4])
oll.add(1, 2)  # Ajouter 2 à la position 1
assert oll.get(1).value() == 2  # Le deuxième élément doit être 2
assert oll.get(0).value() == 1  # Le premier élément doit être 1
assert oll.get(2).value() == 3  # Le troisième élément doit être 3