...
class OrderedLinkedList(LinkedList):
    ...
    def linkedlist_to_list(self):
        liste = []
        cargo = self.first()
        liste.append(self.first().value())

        while cargo is not None:
            liste.append(cargo.value())
            cargo = cargo.next()
        return liste
    
    def ordererd_liste(self):
        lst = OrderedLinkedList.linkedlist_to_list(self)
        lst = sorted(lst)
        lst.reverse()
        return lst
    
    def list_to_orderedlinkedlist(self):
        lst = OrderedLinkedList.ordererd_liste(self)
        LinkedList(lst)
    
    def add(self,cargo):
        super().add(cargo)
        self.list_to_orderedlinkedlist()
